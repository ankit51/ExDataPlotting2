...
par(mfrow=c(2,2))
with(hpc3, plot(datetime, Global_active_power, type="l",
                ylab='Global Active Power (kilowatts)',xlab=''))
with(hpc3, plot(datetime, Voltage, type="l",ylab='Voltage'))
with(hpc3, plot(datetime, Sub_metering_1, type="l",ylab='Energy sub metering',xlab=''))
lines(datetime, Sub_metering_2, col='red')
lines(datetime, Sub_metering_3, col='blue')
legend("topright",lwd=1,cex=0.4, col=c('black','red','blue'),
       legend=c('Sub_metering_1','Sub_metering_2','Sub_metering_3'))
with(hpc3, plot(datetime, Global_reactive_power, type="l"))
dev.off()
detach(hpc3)